...
# The compare package, named once: six files (its shared half moved to
# src/agents/analysis/compare_common.py on 2026-08-18) and the
# per-observable sub-boxes reference it on nearly every line.
CMP = "mcp/elm-mcp/src/compare/"
# The shared half — loading, pairing, metrics, the driver — is the FRAMEWORK's.
CC = "src/agents/analysis/compare_common.py"
...
BLURB = {
    ...
    "hydrodata":  "Water table from HydroFrame, two kinds, never merged: the "
                  ...
                  "basin. Since 2026-08-17 it also writes the CONUS2 SUBSURFACE — five fields to 392 m, once per basin as .npz/.json beside reception.json — which the PFLOTRAN server joins at deck build. Catalogue is open; data needs a PIN.",
    "daymet":     "Daily precipitation at every grid point of the basin, 1 km, "
                  "1980 onward (ORNL). Written into reception.json once per "
                  "run as a series per point; PFLOTRAN's build reads it as "
                  "the top boundary of each column, and an off-grid column "
                  "borrows the nearest point's series within one grid spacing.",
...
GROUPS = [
 {"name": "Reception", "note": "one sentence in, a brief out", "steps": [
  {"n": "Reception", "side": "fw",
   "sum": "Parse the request, name the model; gather the basin, its rain, its subsurface and every nearby station.",
   "flow": [ref("An LLM turns the sentence into a <b>brief</b>: basin, year, what is being asked. <b>Two MCP tools, plus three local ones</b> (<code>list_servers</code>, <code>describe_server</code>, <code>describe_sweep</code>) so the brief also NAMES THE MODEL, chosen by reading the servers rather than by a menu written before the request. Everything else is fetched by code afterwards, because what to fetch is not a decision — with twelve tools the loop spent most of its time choosing them and then summarised a truncated result.", "src/agents/reception_llm.py", "LLMReceptionAgent"),
            ...
            ref("<b>The model is named by reading the servers</b>: three local tools list what is configured, ask a model server for its report (<code>describe_*_capabilities</code>) and its sweep menu; the brief writes the model, the reason, and whether it can run.", "src/core/model_servers.py", "describe_server"),
            ref("Daily rain at every grid point, once per run — Daymet, on the run's own years.", "src/core/data_gather.py", "gather_precipitation"),
            ref("The CONUS2 subsurface — five fields to 392 m — once per basin, as arrays beside reception.json; the PFLOTRAN server reads it at deck build.", "src/core/data_gather.py", "gather_subsurface"),
   ...
   "files": f("src/agents/reception_llm.py", "src/core/data_gather.py",
              "src/core/model_servers.py", "src/core/static_wtd.py",
 ...
 {"name": "Planner", "note": "the brief becomes a design", "steps": [
  {"n": "Planner", "side": "fw",
   ...
   "flow": [ref("Read the brief.", "src/agents/planner.py", "plan"),
            ref("Two blocks from the ONE server the planner touches, read once and memoized: <code>pinning</code> (what a column of this model may be compared against) and <code>constraints</code> (what a plan may not vary), appended to the system prompt under labelled headings.", "workflow.py", "_pinning_block"),
            ...
            ref("Emit the plan; <code>workflow._plan</code> writes <code>strategy.json</code> with the chosen server's <code>pinning</code> block beside it — the rulebook travels with the design. The strategy check runs at materialize, the last moment both files are in hand.", "src/core/strategy_check.py", "check")],
   "files": f("src/agents/planner.py", "src/agents/prompts/planner.txt",
              "src/core/strategy_check.py")}]},

 {"name": "Experiment Manager", "note": "design → inputs → cases → run → results — one sequence, a manager per model beside its server",
  "steps": [
  {"n": "Dispatch", "side": "fw", "sum": "The coordinator runs the model reception chose, in a directory named for it.",
   "flow": [ref("Reception names the model; the coordinator checks it against what can run — <code>RUNNABLE</code>, ELM and PFLOTRAN — and adopts it. A model that describes itself but cannot be driven is refused with the package saved.", "workflow.py", "_adopt_model"),
            ref("The run directory is minted FROM THE BRIEF, between reception's LLM phase and its gather: <code>{model}_run_{stamp}</code>. Reception is handed a function, calls it once the model is known, and gathers into it; a clarification mints nothing.", "workflow.py", "process_request"),
            ref("The manager for the model, imported by path from beside its server — <code>mcp/elm-mcp/src</code>, <code>mcp/pflotran-mcp</code> — the one place that knows where a manager lives. <code>src/core</code> names no model.", "src/core/resumable.py", "_manager_for"),
            ref("ELM's config carries a period in years and a warm start; any other model takes the base config plus the resolved years.", "workflow.py", "_config_for"),
            ref("<code>--request TEXT</code> runs one study unattended, reception through the Analyzer; <code>--interactive</code>, <code>--resume</code> and <code>--finalize</code> as before. Every path the coordinator hands a server is absolute — the servers have their own working directories.", "workflow.py", "main")],
   "flag": "Until 2026-08-18 _execute named ELM's manager by hand and every run directory was elm_run_*: a PFLOTRAN study stopped at dispatch with 'cannot run a study with it yet' on the same afternoon a hand-driven one ran through package.",
   "files": f("workflow.py", "src/core/resumable.py", "src/core/model_servers.py")},
  {"n": "Sample columns", "side": "fw", "sum": "The plan becomes real coordinates.",
   "flow": [ref("Take reception's grid AS WRITTEN — already clipped to the watershed polygon; the sampler fetches and clips nothing (2026-08-18).", "tools/expand_sampling.py", "expand"),
            ...
            ref("Hand the columns to the build call as DATA. <code>columns.json</code> is written by the base only AFTER that call has written what it learned onto them, and it is the ONE record the package reads.", "tools/expand_sampling.py", "expand")],
   "flag": "Sampling is entirely FRAMEWORK-SIDE, a pure function of reception.json + strategy.json. The server never chooses a coordinate: ELM's build call SNAPS these to donor gridcells, PFLOTRAN's leaves them and writes the site join onto each. Either way columns.json is written after that call, and the design figure is drawn from it by code beside each server.",
  ...
  {"n": "Build inputs", "side": "srv", "sum": "One call: six steps, ending in case_inputs.json.",
   "flow": [ref("<b>Warm start</b> — subset the CONUS restart per column, <b>snapping each to its donor gridcell</b>.", "mcp/elm-mcp/src/inputs.py", "warm_start"),
            ...
            ref("Write <code>case_inputs.json</code>, and <code>elm_columns.json</code> as the server's own copy of the snapped columns; the framework's <code>columns.json</code>, written after this call, is the record the package reads.", "mcp/elm-mcp/src/inputs.py", "write_case_inputs"),
  ...
  {"n": "Build decks", "side": "srv", "sum": "PFLOTRAN's one build call: the server joins the site data and writes a deck per column.",
   "flow": [ref("ONE call at materialize — the slot ELM uses for its warm start: <code>create_decks_from_columns(columns, out_dir, site_dir=run_dir)</code>. The columns come back AS BUILT and replace the list in place, so <code>columns.json</code> carries what the server learned.", "mcp/pflotran-mcp/pflotran_exp_manager.py", "_refine_columns"),
            ref("The server reads the CONUS2 subsurface, the water-table raster and reception's rain ITSELF and joins them per column: no interpolation, no clamping. An off-grid column borrows the nearest grid point's rain within one grid spacing and says how far (<code>rain_borrowed_km</code>).", "/qfs/people/tran289/IDEAS/reaction_sandbox_mcp-upstream/tools/site_data.py", "join"),
            ref("The domain ends on the shallowest CONUS2 layer boundary below the water table; a column with under 0.5 m of unsaturated depth carries a <code>warning</code>, as does one built steady with no daily rain — the class of problem in the sentence, the number in the field.", RXN, "create_decks_from_columns"),
            ref("The server's run plan — CONDITIONS_COUPLERS, PFLOTRAN_CONFIG, the assumptions ledger, the incomplete columns — handed back to the base, exactly as ELM's is.", "mcp/pflotran-mcp/pflotran_exp_manager.py", "_to_run_plan"),
            ref("What this build ADDS to a column, declared beside the server and composed with the base's <code>COLUMN_METADATA</code>; a key neither names raises at package rather than vanishing.", "mcp/pflotran-mcp/pflotran_exp_manager.py", "COLUMN_METADATA_EXTRA"),
            ref("<b>A controlled sweep takes the same slot with no site</b>: <code>_build_sweep_columns</code> checks the design (<code>check_conceptual_design</code>) then builds columns (<code>build_conceptual_columns</code>) that already carry a textbook soil, a water table and their rain, so <code>_refine_columns</code> passes NO site_dir — the sweep IS the site path from here. The ELM manager's shape, so a refusal names every reason at once.", "mcp/pflotran-mcp/pflotran_exp_manager.py", "_build_sweep_columns"),
            ref("<b>A COUPLED run takes the same slot with the PRIOR RUN'S columns, verbatim</b> — same ids, coordinates, bands, pinned stations, so the two models line up column by column. <code>_build_coupled_columns</code> reads the prior run's RECORD (experiment.json + extracted.json — no NetCDF reopened, no server crossed): each column carries its own daily QDRAI series (<code>daily_flux_mm_day</code>) and ELM's own solved water table as its anchor; <code>flux_description</code> becomes the deck's forcing_caveat. Reception's coupling archetype gathers NOTHING new — the prior's grid, observations and site files are CARRIED FORWARD and the provenance says so. Replaces step 4d (a PFLOTRAN appendix on the ELM run's tail, through a local deck builder that bypassed the server — deleted 2026-08-19).", "mcp/pflotran-mcp/pflotran_exp_manager.py", "_build_coupled_columns"),
            ref("<b>TWO-WAY is the same archetype pointed back</b>: the ELM manager's <code>_build_coupled_columns</code> hands each column PFLOTRAN's SOLVED water table (the pressure crossing at the final time, read by the one public function that owns that definition, <code>compare/water_table.solved_water_table_m</code>) as its next initial state — <code>initial_water_table_m</code>, with <code>water_table_delta_m</code> the number an iteration watches. The warm start stamps it into the fresh finidat (<code>set_water_table.py</code>: ZWT and WA consistent by ELM's own relation zwt = 28.802 − wa/200, recovered from the CONUS restart itself; clamps at ELM's 28.802 m aquifer bottom reported, in-soil targets marked approximate). Iterate ELM ⇄ PFLOTRAN until the deltas are small — Picard at whole-run granularity, and the WHOLE LOOP FITS IN ONE ALLOCATION: <code>tools/coupling_loop.sh</code> is sbatch'd once; inside it the ELM manager notices SLURM_JOB_ID and runs its ensemble in place (<code>run_elm_ensemble(in_allocation=true)</code> — the wrapper was always srun-based), the PFLOTRAN legs are inline as ever, and <code>tools/coupling_delta.py</code> checks convergence off each PFLOTRAN leg's record before the next ELM minute is spent.", "mcp/elm-mcp/src/elm_exp_manager.py", "_build_coupled_columns"),
            ref("<code>sampling_design.png</code>: on a site run, the columns over the terrain, the rain at each and through the year, the CONUS2 water table each was given, its depth on the layer ladder, its unsaturated column. On a SWEEP — no place — four panels instead: the design table, each column on the layer ladder with its water table, the written rain or steady rate, the retention curve of each texture class.", "mcp/pflotran-mcp/sampling_design.py", "render_run")],
   "flag": "The join used to be the framework's — a 117-line _to_run_plan handing the server pre-chewed columns — and the deck facts travelled by a second, unchecked list that lost 'this column is essentially saturated' on the way to experiment.json. One seam now: columns.json.",
   "files": f("mcp/pflotran-mcp/pflotran_exp_manager.py", "mcp/pflotran-mcp/sampling_design.py",
              "mcp/pflotran-mcp/README.md")},
  ...
  {"n": "Run and read (PFLOTRAN)", "side": "fw", "sum": "Inline, one deck per call; then the raw series.",
   "flow": [ref("<code>run_pflotran_simulation</code> once per deck — the tool's list form runs only the first — seconds to a minute and a half each, inline, no scheduler; a failed column is recorded against itself and the ensemble goes on. Status and runtime are written onto the experiments in place, which is where RUN_SUMMARY.json reads them.", "mcp/pflotran-mcp/pflotran_exp_manager.py", "_run"),
            ref("<code>extract_column_series</code>: saturation and liquid pressure against depth at each output time, to <code>03_results/extracted.json</code>; each column's block rides onto its row as <code>profiles</code>. Raw numbers only — front depth, transit time and fluxes are the Analyzer's to derive.", "mcp/pflotran-mcp/pflotran_exp_manager.py", "_extract"),
            ref("What each field MEANS — saturation is of pore space, pressure is absolute, the times include the spin-up, the water table is an INPUT — stays on this side of the boundary, never behind a tool call.", "mcp/pflotran-mcp/pflotran_exp_manager.py", "FIELD_SEMANTICS")],
   "files": f("mcp/pflotran-mcp/pflotran_exp_manager.py")},
  ...
  {"n": "Package", "side": "fw", "sum": "Rows become the bundle the Analyzer reads.",
   "flow": [ref("Assemble rows into the standard result shape.", "src/core/exp_manager_base.py", "_package"),
            ref("The framework's list plus the manager's additions, composed once; the merge asserts against the union.", "src/core/exp_manager_base.py", "_column_keys"),
            ref("<b>Join the column metadata onto the rows — and assert first.</b> <code>COLUMN_METADATA</code> names what the SAMPLER writes onto columns.json — where a column is, its band, its station, its years, a sweep's treatment — and each manager declares what its BUILD CALL adds as <code>COLUMN_METADATA_EXTRA</code> beside its server (ELM: donor soil, forcing cell, warm start; PFLOTRAN: water table, unsaturated depth, transient or steady, the deck's own warning). <code>_column_keys</code> composes the two once. Every key is either kept or dropped with a reason; several are OPTIONAL because a site run has no treatment and a column on the grid has no borrowed rain, and neither absence is a hole. A key in neither list is not a decision: it raises, says which key and where the list lives, instead of dropping the value in silence — which is how a per-column warning vanished before 2026-08-18.", "src/core/exp_manager_base.py", "_merge_column_metadata"),
 ...
 {"name": "ELM MCP", "note": "11 tools · 19 modules · 7 scripts — the whole ELM simulation",
  "steps": [
  {"n": "Tools", "side": "srv", "sum": "The eleven calls the framework may make (three of them for a controlled sweep).",
   "flow": [ref("<code>describe_elm_capabilities()</code> — the workflow, a CHECKED inventory of every external dependency, and what this server does not do. Exhaustive, and a test enforces that against the registry.", ELM_MAIN, "describe_elm_capabilities"),
            ...
            ref("<code>get_column_metadata(run_dir)</code> — the server's copy (<code>elm_columns.json</code>) of the columns as they will be RUN. The framework's columns.json carries the same post-snap columns since the build call writes onto them in place; it is the record the package reads.", ELM_MAIN, "get_column_metadata"),
  ...
  {"n": "Input build", "side": "srv", "sum": "Locations in → a runnable case list out.",
   "flow": [ref("Snap each column to its CONUS donor gridcell and subset the restart → per-column <code>finidat</code>.", "mcp/elm-mcp/src/inputs.py", "warm_start"),
            ...
            ref("Write <code>case_inputs.json</code> + <code>elm_columns.json</code>, the server's copy of the snapped columns (the framework's columns.json is the record the package reads).", "mcp/elm-mcp/src/inputs.py", "write_case_inputs")],
  ...
  {"n": "Compare to obs", "side": "srv", "sum": "Model beside observations. Numbers only.",
   "flow": [ref("Reuse <code>03_results/extracted.json</code> when it is already there; call the extract tool when it is not. ONE artifact, written by every path that reads history files.", ELM_MAIN, "_extracted_rows"),
            ref("Read the observations out of <b>reception.json in the shape reception wrote it</b> — the only source. A tool that re-fetched them into a CSV returned 14,102 values identical to what was already on disk, and was deleted. Unparseable rows are DROPPED AND COUNTED, never coerced to zero.", CC, "load_observations"),
            ref("<b>One module per observable</b>, each a SPEC + compare() + plot(); the package is only the REGISTRY (<code>OBSERVABLES</code>) and the driver — compare, draw, write comparison.json, summarise — is the framework's, generic over any registry (moved 2026-08-18 so PFLOTRAN's package shares it). A fifth observable is a file and one registry line.", CMP + "__init__.py", "compare_all"),
            ...
            ref("<b>A designed PIN wins outright</b> — where the sampler placed a column at a station, that pairing is honoured with no distance or elevation test. Re-deriving it geometrically was guessing at an answer already recorded in <code>columns.json</code>.", CC, "pair_stations"),
            ref("For the rest: a column qualifies on BOTH limits, then the CLOSEST wins. Elevation FILTERS, distance DECIDES. Limits are <b>per observable</b> and declared on the Spec — SWE is 150 m and 5 km. One pass over the columns in name order, and a station leaves the pool when taken, so col_01 has first refusal.", CC, "pair_stations"),
            ref("An unpaired station carries the number that disqualified it, and <b>&quot;no coordinates&quot; is its own reason</b> — collapsing it into &quot;no column qualified&quot; is what let 236 coordinate-less wells read as a pairing failure.", CC, "pair_stations"),
            ref("Then the numbers, the same for all four: inner-join model and observation <b>on the date, never interpolated</b>, and report bias / MAE / RMSE / r / NSE / KGE with the measured-versus-gap-filled split beside them.", CC, "metrics")],
   "subs_label": "One module per observable",
   "subs": [
     {"n": "swe", "sum": "H2OSNO against a snow pillow. MATCHES FIRST, then compares.",
      "flow": [ref("<b>Match, then compare</b> — the first observable to; water_table and et followed on 2026-08-12. Every station used to be compared against every column: 8,772 paired days on a 17-column Naches run to keep 516. The cost was never the arithmetic, it was that the record then carried 34 complete metric sets beside one &quot;assignment&quot;, and the flattering one is always in there somewhere.", CMP + "swe.py", "compare"),
     ...
     {"n": "water_table", "sum": "ZWT against recorder wells, and against two static fields.",
      "flow": [ref("Recorder wells differenced against the model, on a log axis, with the 3.8 m active-soil count.", CMP + "water_table.py", "compare"),
               ref("<b>Three DISTRIBUTIONS, never merged</b>: the model's mean ZWT per column, Fan 2013 (MEASURED, one long-term mean per in-basin well) and ParFlow CONUS2 (SIMULATED, read at each column from the GeoTIFF through <code>static_wtd.describe</code>). Model and ParFlow sit at the same points; Fan is a basin distribution only.", CMP + "water_table.py", "_distributions")],
     ...
     {"n": "map", "sum": "Where we are even looking. All observables, one figure.",
      "flow": [ref("<b>One ROW per observable, observation left, ELM right</b>, values coloured on a scale per row (never per figure), over a hillshade with the basin divide. Each row's points come from that observable's own <code>map_points()</code>, so the map cannot disagree with the record beside it.", CMP + "maps.py", "plot_all"),
               ref("Water table has three sources for two slots: Fan takes the panel and the recorder wells ride on top as an overlay sharing the row's scale; a basin with no well gets no overlay rather than an empty panel.", CMP + "maps.py", "plot_all")],
      "flag": "Snow pairs on ELEVATION and two columns adjacent on this map can be 1,000 m apart vertically — the hillshade hints at it, the record's separation and elevation delta say it.",
      "files": f(CMP + "maps.py")}],
 ...
 {"name": "PFLOTRAN MCP", "note": "48 tools · reactive transport, ensembles, and the LAMBDA network — the framework uses four",
  "tag": "server outside this repo", "steps": [
  {"n": "Decks", "side": "srv", "sum": "Write and check a PFLOTRAN input deck.",
   "flow": [ref("<code>create_decks_from_columns</code> — THE FRAMEWORK'S BUILD: given the sampled columns and the run directory, join the CONUS2 subsurface, the water table and the rain per column and write one deck each; return the run plan. Also <code>describe_pflotran_capabilities</code>, the pinning and constraints report reception and the planner read.", RXN, "create_decks_from_columns"),
            ref("<code>create_pflotran_input</code> — a deck SKELETON (grid, time, output; no physics) — it says runnable=False.", RXN, "create_pflotran_input"),
  ...
  {"n": "Run", "side": "srv", "sum": "Inline, one deck per call — the server has no scheduler path.",
   "flow": [ref("<code>run_pflotran_simulation</code> — 1-D columns take seconds, so this runs INLINE and returns the result, no job id; num_cores defaults to 1.", RXN, "run_pflotran_simulation"),
            ref("<code>check_simulation_status</code> — completed / running (with progress) / failed, read off what PFLOTRAN v7 actually prints.", RXN, "check_simulation_status"),
            ref("<code>create_parameter_ensemble</code> — LHS/Sobol parameter sets for an ensemble.", RXN, "create_parameter_ensemble")],
   "flag": "The opposite contract to ELM's: PFLOTRAN is fast enough to answer in the call, so ELM has only a job path and PFLOTRAN only an inline one. The server's scheduler trio (submit/check/collect) was ours, unused, and deleted 2026-08-18.",
   "files": [[RXN, lines(RXN)]]},
  {"n": "Results", "side": "srv", "sum": "Observations out of finished runs.",
   "flow": [ref("<code>extract_column_series</code> — THE FRAMEWORK'S READ: saturation and pressure against depth per output time, per column, to a file; summaries in the reply.", RXN, "extract_column_series"),
 ...
 {"name": "Analyzer", "note": "results become an answer", "tag": "being redesigned",
  ...
  {"n": "Compare to obs", "side": "fw", "sum": "Call the model's comparison; judge what it may support.",
   "flow": [ref("The measurements come from the MODEL'S server — the run's model chooses its comparison PACKAGE, loaded by path from beside its server as <code>compare_&lt;model&gt;</code> (ELM: four observables; PFLOTRAN: water_table); the driver — compare, draw, write, summarise — is the framework's <code>compare_common.compare_run</code>, one call, the figures with it. Ref: src/agents/analysis/step1_compare.py::_load_compare. The 1,400 lines that used to do this here knew H2OSNO from ZWT and how deep ELM's soil goes, in the one box that also reads PFLOTRAN runs.", "src/agents/analysis/step1_compare.py", "compare_all"),
            ref("The shared half — reading reception's observations, pairing stations to columns by pin and by distance, metrics, figures, and the driver that compares, draws, writes comparison.json and summarises — is the FRAMEWORK'S, generic over a registry; each model keeps only its observables beside its server.", CC, "compare_run"),
            ref("PFLOTRAN's one observable: the depth where the solved pressure crosses atmospheric — found BOTTOM-UP, so a wet pocket at the surface is not read as the water table — and the CONUS2 depth the column was given, against a well's daily series on the snapshot dates and as a level. The bottom face is a fixed head; the water table is solved and moves, and the record says how far.", "mcp/pflotran-mcp/compare/water_table.py", "compare"),
            ref("A basin with no recorder well can still show the machinery: a synthetic well is written into a COPY of the run, labelled synthetic on the record, in quality and in provenance, and never near the run itself.", "tools/synthetic_well.py", "make_well"),
   ...
   "files": f("src/agents/analysis/step1_compare.py", CC)},
  {"n": "Interpret", "side": "fw", "sum": "What the numbers mean, and whether to trust them.",
   "flow": [ref("Build the context: results, assumptions ledger, and the limitations catalogue.", "src/agents/analysis/step0_context.py", "load"),
            ref("<b>The split is declared, not implied.</b> experiment.json has 21 top-level keys; 11 become <code>data</code> — the only place a reported number may come from — and the other 10 SAY WHERE THEY WENT instead, 4 to <code>plan</code> and 3 to <code>caveats</code>. This is the list that dropped <code>spinup_dropped</code> for six days while the value sat in the file and a reader waited for it in step 4.", "src/agents/analysis/step0_context.py", "_EXPERIMENT"),
            ref("<b>Structural caveats are promoted to blocking</b> — they do not qualify a gauge comparison, they forbid the naive one. A row's <code>warning</code> — the server's own sentence about a column, carried through the asserted seam — becomes one qualify caveat per distinct sentence, naming the columns; still three files, model-blind.", "src/agents/analysis/step0_context.py", "_caveats"),
            ...
            ref("The brief names the run's model and the catalog says WHERE each declared field lives — a variable of <code>prof</code>, an axis, an INPUT on the row, or in <code>metrics</code> — read off the rows rather than assumed, so a PFLOTRAN input is not sought in an empty key.", "src/agents/analysis/step2_investigate.py", "_variable_catalog"),
            ...
            ref("<b>The plan is written down.</b> <code>investigation_plan.md</code> records how the question was mapped onto the model's variables, what each figure was meant to answer, and — rendered AFTER the scripts run — what each one actually returned. Step 3 reads it, so a reviewer judging whether a figure set answers the question knows what it was trying to answer.", "src/agents/analysis/step2_investigate.py", "render_plan_md"),
