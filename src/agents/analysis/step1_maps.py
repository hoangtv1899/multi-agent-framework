...
src/agents/analysis/step1_maps.py
    ...
    in   the three step-1 comparison records + ctx
    out  one grid: a row per observable, non-ELM source left, ELM right

WHY IT IS A COMPARISON AND NOT A VALIDATION. Validation would mean measuring
the model against measurements of the same quantity in the same place. Almost
none of that holds here: 3 of 5 SNOTEL sites, 11 of 19 gauges and ALL TEN wells
lie outside the watershed that was simulated; a gauge integrates a routed
catchment while a column is 1 m2 of unrouted local generation; and Fan 2013 is
itself a model. What the figure supports is a comparison of fields that
overlap partially and on different terms, which is worth drawing and is not
the same claim.

WHY THIS IS ITS OWN MODULE. It reads all three comparison records, so it cannot
live inside any one of them without that module importing its siblings. Every
point it draws comes from the comparison modules' own map_points(), so this
file decides LAYOUT and nothing else — it never recomputes a mean, a
coordinate, or an exclusion. If this figure and a comparison module ever
disagree, the bug is in the module they share, not here.
...
WHAT THE ROWS ARE, AND WHAT THEY ARE NOT.
    ...
    streamflow   USGS vs ELM, both mm/day of specific discharge. The units
                 ...
                 routing and no run-on. See the module's
...
Rows whose comparison returned nothing are dropped, not drawn empty.
"""
...
def create_comparison_spatial_map(ctx, out_path,
                                  swe: Optional[Dict[str, Any]] = None,
                                  streamflow: Optional[Dict[str, Any]] = None,
                                  wtd: Optional[Dict[str, Any]] = None,
                                  **kw) -> str:
    ...
    Same points and same exclusions as the comparison modules computed, in one
    frame. Written as comparison_spatial_map.png.
    """
