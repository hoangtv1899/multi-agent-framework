...
| `pflotran` | materialize → build → run (direct, ~1 s/column) → extract | hydrostatic at the Fan 2013 water table | `pflotran_run_*` |
| `lambda-pflotran` | as `pflotran`, plus a chemistry graft on each deck | as `pflotran` | `lambda_pflotran_run_*` |

All three write the same `experiment.json` and are read by the same Analyzer.
The PFLOTRAN backends add a `profiles` block (depth × output time); they have
no daily series, so `step0_context.series()` returns None and `profiles()`
carries the data instead.

### `lambda-pflotran` — what it is, and what it is not

A **demonstration** of reactive transport, not a calibration. The reaction
parameters come verbatim from the build's verified regression case, and the
reaction network is generated by binning the sample's FTICR table into 3–5
organic-matter pools — a binning its own header marks as RECONSTRUCTED. Every
one of these facts reaches the Analyzer through the assumptions ledger.

Two constraints are enforced by the manager, not left to the user:

- **1 year, not the parent's 20.** On sampled columns the timestep collapses to
  machine epsilon between t = 1.02 and 1.79 y. That is not the domain depth
  (identical at 50 m and 12 m), not the saturation (columns with a water table
  fail too), and not the generated network (the build's own regression network
  fails on the same column). Every collapse observed is above 1.0 y and at
  1.0 y every water-bearing column completes. A longer request is clamped and
  the clamp is recorded.
- **Only columns with a water table in the domain.** A fully unsaturated column
  has no aqueous phase to transport solutes through. Those columns get no
  reactive deck, and the ledger names them — on an 8-column Gunnison sample
  that was 5 of 8.

Because of the first constraint, a `lambda-pflotran` water table has relaxed
one year and a `pflotran` one twenty. They are different experiments and the
ledger says so.
